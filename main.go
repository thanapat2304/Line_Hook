...
func main() {
	...
	r.POST("/iot-alert", func(c *gin.Context) {
		...
		// ไม่ส่งข้อความถ้า status เป็น "เซนเซอร์มีปัญหา"
		if req.Status == "เซนเซอร์มีปัญหา" {
			fmt.Println("==== SKIP SENDING MESSAGE FOR SENSOR ISSUE ====")
			c.JSON(http.StatusOK, gin.H{"status": "message skipped"})
			return
		}
